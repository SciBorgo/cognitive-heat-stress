...
gather_draws(fit_con, b_con_ies_first) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
...
gather_draws(fit_con, b_polytcoredegreeEQ21) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
gather_draws(fit_con, b_polytcoredegreeEQ22) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
# Plot: b_glucose
gather_draws(fit_con, b_glucose) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
# Plot: b_blood_osmo
gather_draws(fit_con, b_blood_osmo) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
# Plot: b_suit2:wbgt30
gather_draws(fit_con, `b_suit2:wbgt30`) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
# Plot: b_suit4:wbgt30
gather_draws(fit_con, `b_suit4:wbgt30`) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
# Plot: b_suit2:wbgt37
gather_draws(fit_con, `b_suit2:wbgt37`) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
# Plot: b_suit4:wbgt37
gather_draws(fit_con, `b_suit4:wbgt37`) %>% 
  mutate(effect = .value) %>%
  ggplot() + 
  stat_halfeye(aes(x = effect), .width = c(0.66, 0.95)) + 
  theme_bw(base_size = 10) + 
...
plot_grid(plot_initial_ies,
          ...
          scale = 0.95, 
          align = 'v', axis = "lr")
ggsave(file = "congruent-parameter-estimate-plot.png", units="in", width = 9.5, height = 12, dpi = 300)
