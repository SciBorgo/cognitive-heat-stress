...
dat = read.csv("Cognitive data_04-02-2020.csv") %>%
  clean_names()
...
ggplot() +
  ...
  #geom_point(data = d, aes(x = tcore, y = con_ies)) +
  scale_y_continuous(limits = c(650,830), n.breaks = 8) +
  scale_x_continuous(n.breaks = 8) +
  labs(x = "Core Body Temperature (°C)", y = "Inverse Efficiency Score (ms)") +
  facet_grid(~"Effect of Core Body Temperature on\n Congruent Performance") +
...
ggplot() +
  ...
  geom_ribbon(data = df_66, aes(x = (tcore*sigma)+mu, y = estimate__, ymin = lower__, ymax = upper__), alpha = 0.5) +
  geom_line(data = df_95, aes(x = (tcore*sigma)+mu, y = estimate__)) +
  #geom_point(data = d, aes(x = tcore, y = incon_ies)) +
  scale_y_continuous(limits = c(650,830), n.breaks = 8) +
  scale_x_continuous(n.breaks = 8) +
  labs(x = "Core Body Temperature (°C)", y = "Inverse Efficiency Score (ms)") +
...
plot_grid(plot_con,
          ...
          labels = c('A','B'),
          label_size = 16)
ggsave(file = "marginal-effect-tcore-plot.png", units="in", width = 8, height = 3.5, dpi = 300)
